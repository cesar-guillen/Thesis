...
void receive_hash(size_t msg_length, uint8_t* buffer) {
  ...
  int result = decrypt_message((char*)encrypted_hash, clen, decrypted_hash,
                             &decrypted_len, current_nonce,
                             hash_code, HASH_SIZE);
  if (result < 0) {
...
int decrypt_request(char* ciphertext, size_t clen, char* plaintext, const unsigned char* nonce) {
  ...
  // Estimate expected message length (ciphertext - tag size)
  size_t mlen_expected = clen - 16;
  int result = decrypt_message(ciphertext, clen, plaintext, &decrypted_mlen,
                               nonce,msg_code , mlen_expected);
